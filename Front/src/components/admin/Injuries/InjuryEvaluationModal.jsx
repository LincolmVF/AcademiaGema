...
const InjuryEvaluationModal = ({ isOpen, onClose, solicitud, onEvaluateSuccess }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#0f172a]/70 backdrop-blur-md animate-in fade-in duration-300">
            {/* Contenedor con Scroll Interno para evitar que el botón se pierda */}
            <div className="bg-white border border-slate-200 rounded-[2.5rem] w-full max-w-lg shadow-2xl relative animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                
                {/* Botón Cerrar - Fijo arriba */}
                <button onClick={onClose} className="absolute top-6 right-6 text-slate-400 hover:text-[#1e3a8a] z-10 transition-colors p-2 hover:bg-slate-50 rounded-full">
                    <X size={20} />
                </button>
                ...
                <div className="p-8 overflow-y-auto custom-scrollbar">
                    ...
                    <form onSubmit={handleEvaluate} className="space-y-6">
                        ...
                        {/* Botón de envío - Siempre visible por el scroll del contenedor */}
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-[#1e3a8a] hover:bg-[#162a63] text-white font-black uppercase tracking-[0.2em] py-4 rounded-2xl shadow-xl shadow-blue-900/20 transition-all disabled:opacity-50 active:scale-95 text-[11px]"
                        >
