...
const WeeklyTimelineDashboard = () => {
  ...
  // 3. MATEMÁTICAS DEL CALENDARIO (A prueba de errores)
  ...
  const getCardStyle = (horaInicio, horaFin) => {
    ...
    // Distancia desde el borde superior (8:00 AM)
    ...
    return { 
      top: `${topPx}px`, 
      height: `${Math.max(heightPx, 30)}px` // Mínimo 30px para que no se vea aplastado
    };
  };
  ...
  return (
    <section className="bg-white rounded-[2rem] md:rounded-[3rem] p-4 md:p-6 shadow-xl border border-slate-100 flex flex-col">
      ...
      {/* CONTENEDOR PRINCIPAL - DISEÑO CALENDARIO PROFESIONAL */}
      <div className="border border-slate-200 rounded-2xl bg-white overflow-hidden flex flex-col shadow-sm">
        
        {/* Cabecera Fija de Días */}
        <div className="flex border-b border-slate-200 bg-slate-50 shrink-0 overflow-hidden">
        ...
        {/* Cuerpo Scrollable del Calendario */}
        <div className="flex-1 overflow-auto max-h-[400px] custom-scrollbar bg-slate-50/30">
          <div className="flex relative min-w-[600px]" style={{ height: `${TOTAL_HOURS * ROW_HEIGHT}px` }}>
            
            {/* GRID DE LÍNEAS DE FONDO (Perfectamente precisas) */}
            ...
            <div className="w-14 border-r border-slate-200 shrink-0 bg-white relative z-10">
                    ...
                    {clasesDelDia.map((registroAsistencia, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Contenido de la Tarjeta (Ref visual de la imagen) */}
