...
const EditTeacherProfileModal = ({ isOpen, onClose, onSuccess, currentUser }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiFetch.put(API_ROUTES.AUTH.UPDATE_PROFILE, payload);
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-2 sm:p-4 bg-[#0f172a]/90 backdrop-blur-sm">
      <div className="bg-white w-full max-w-md rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto p-5 custom-scrollbar">
          <form id="profile-teacher-form" onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="p-4 border-t border-slate-100 bg-white shrink-0">
          <button form="profile-teacher-form" type="submit" disabled={loading} className="w-full bg-[#1e3a8a] hover:bg-[#0f172a] text-white font-black py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 shadow-md uppercase tracking-wider text-xs">
