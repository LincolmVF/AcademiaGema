    ...
    return (
        <div className="space-y-6 animate-fade-in-up p-1">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-black text-slate-900 uppercase">Niveles <span className="text-[#1e3a8a]">Académicos</span></h1>
                <button onClick={() => setView('create')} className="bg-[#1e3a8a] text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredNiveles.map((nivel) => (
                        <div key={nivel.id} className="bg-white rounded-3xl border border-slate-200 p-6 hover:shadow-xl transition-all group">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <div className="flex gap-1">
                                    <button onClick={() => handleEdit(nivel)} className="p-2 text-slate-300 hover:text-[#1e3a8a]"><Edit3 size={18} /></button>
                                    <button onClick={() => handleDelete(nivel.id)} className="p-2 text-slate-300 hover:text-red-500"><Trash2 size={18} /></button>
                                </div>
                            </div>
                            <h3 className="font-black text-slate-800 text-lg uppercase italic mb-4">{nivel.nombre}</h3>
                            <div className="pt-4 border-t border-slate-50 flex justify-between items-center">
                                <span className="text-green-600 font-black">S/ {Number(nivel.precio_referencial).toFixed(2)}</span>
